...
function extractJsonFromZip(zipPath, outPath, preferredName = "") {
  const py = String.raw`
    ...
    if target is None:
        for n in names:
            if n.endswith('.json'):
                ...
                break
    if target is None:
        raise SystemExit('No JSON file found in artifact zip')
    data = zf.read(target)
...
async function main() {
  ...
  const fileName = String(ref.file_name || "candidates.json").trim();
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ghaw-artifact-"));
  const zipPath = path.join(tmpDir, "artifact.zip");
  await githubDownload(String(artifact.archive_download_url), token, zipPath);
  const info = extractJsonFromZip(zipPath, outFile, fileName);
